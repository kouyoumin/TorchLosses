...
class IoULoss(nn.Module):
    ...
    def __call__(self, input, target):
        ...
        union_p = torch.sum(th_input + th_target - th_input * th_target, self.dim) + self.eps# + 1e-8

        if self.mode != 'pos_only':
            intersection_n = torch.sum((1-th_input) * (1-th_target), self.dim)
            union_n = torch.sum((1-th_input) + (1-th_target) - (1-th_input) * (1-th_target), self.dim) + self.eps# + 1e-8
        
        if self.mode == 'pos_or_neg':
            pos = torch.sum(th_target, self.dim).clamp_(0,1)
            iou_loss = self.pos_weight *  self.class_weight * pos * ((1 - intersection_p / union_p).clamp_(0,1)) + self.neg_weight *  self.class_weight * (1-pos) * ((1 - intersection_n / union_n).clamp_(0,1))
        elif self.mode == 'both':
            ...
            iou_loss = self.pos_weight * self.class_weight * pos * ((1 - intersection_p / union_p).clamp_(0,1)) + self.neg_weight * self.class_weight * neg * ((1 - intersection_n / union_n).clamp_(0,1))
        elif self.mode == 'neg_only':
            #class_weight = class_weight * (torch.amax(th_target, self.dim) > 0.5).float()
            #class_weight[class_weight == 0] = 1
            #assert(class_weight.shape == th_target.shape[:2]) # apply only to positive samples
            neg = 1-torch.sum(th_target, self.dim).clamp_(0,1) if self.ignore_full else torch.ones_like(intersection_n)
            iou_loss = self.class_weight * neg * ((1 - intersection_n / union_n).clamp_(0,1))
            ...
            #class_weight[class_weight == 0] = 1
            pos = torch.sum(th_target, self.dim).clamp_(0,1) if self.ignore_empty else torch.ones_like(intersection_p)
            iou_loss = self.class_weight * pos * ((1 - intersection_p / union_p).clamp_(0,1))
        ...
        if self.reduction == 'none':
            ...
            final_loss = iou_loss.nanmean()
        elif self.reduction == 'sum':
            final_loss = iou_loss.nansum()
        else:
            final_loss = iou_loss
...
def test_iouloss_pos_only_dim23():
    """
    Test IoULoss with dim=(2,3)
    """
    loss = IoULoss(dim=(2,3), mode='pos_only')
    ...
    assert(output[0,1] == 0.75)


def test_iouloss_pos_or_neg_dim23():
    """
    Test IoULoss with dim=(2,3)
    """
    loss = IoULoss(dim=(2,3), mode='pos_or_neg')
    input = torch.zeros(4,2,10,10)
    target = torch.zeros(4,2,10,10)
    input[0,1,0:5,0:5] = 1
    input[1,0,0,0] = 1
    ...
    assert(output.shape == torch.Size([4,2]))
    assert(output[0,1] == 0.75)
    assert(torch.isclose(output[1,0], torch.tensor(1-99/100)))


def test_iouloss_pos_only_dim023():
    """
    Test IoULoss with dim=(0,2,3)
    """
    loss = IoULoss(dim=(0,2,3), mode='pos_only')
    input = torch.zeros(4,2,10,10)
    target = torch.zeros(4,2,10,10)
    input[0,1,0:5,0:5] = 1
    input[1,1,0:2,0:2] = 1
    ...
    assert(torch.isclose(output[1], torch.tensor(1-(25+4)/(100+25))))


def test_iouloss_pos_only_class_weight_dim023():
    """
    Test IoULoss with dim=(0,2,3)
    """
    loss = IoULoss(dim=(0,2,3), class_weight=torch.Tensor([1,2]), mode='pos_only')
    input = torch.zeros(4,2,10,10)
    target = torch.zeros(4,2,10,10)
    input[0,1,0:5,0:5] = 1
    input[1,1,0:2,0:2] = 1
    ...
    #print(output)
    assert(output.shape == torch.Size([2]))
    assert(torch.isclose(output[1], torch.tensor(2*(1-(25+4)/(100+25)))))


if __name__ == "__main__":
    for name, fn in list(globals().items()):
        if name.startswith("test_iouloss_"):
            print(name)
            fn()
